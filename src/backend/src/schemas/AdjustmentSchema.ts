...
export const createAdjustmentSchema = z.discriminatedUnion('type', [
  ...
  z.object({
    type: z.literal('ADD'),
    employee_id: z.number().int().positive('employee_id debe ser un entero positivo'),
    new_timestamp: z.string().datetime({ message: 'new_timestamp debe ser una fecha ISO 8601 válida' }),
  ...
  z.object({
    type: z.literal('EDIT'),
    employee_id: z.number().int().positive('employee_id debe ser un entero positivo'),
    clock_log_id: z.number().int().positive('clock_log_id debe ser un entero positivo'),
    new_timestamp: z.string().datetime({ message: 'new_timestamp debe ser una fecha ISO 8601 válida' }),
    log_type: z.enum(['IN', 'OUT']),
    justification: z.string().min(10, 'La justificación debe tener al menos 10 caracteres'),
  }),
