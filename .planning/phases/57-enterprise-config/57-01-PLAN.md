...
must_haves:
  truths:
    ...
    - "vpg_enterprise table has the 4 new configuration fields"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    3. Update `model vpg_enterprise`:
       - Add `minuteRoundingPolicy MinuteRoundingPolicy @default(EXACT)`
       - Add `roundingPolicyAcknowledged Boolean @default(false)`
       - Add `isCommercialActivity Boolean @default(false)`
       - Add `ordinaryShiftType ShiftType @default(DIURNA)`
  </action>
  <verify>
    <automated>npx prisma validate --schema=src/backend/prisma/schema.prisma</automated>
  </verify>
  <done>schema.prisma updated with new enums and fields, passes validation.</done>
</task>
